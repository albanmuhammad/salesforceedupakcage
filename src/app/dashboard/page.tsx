        ...
        <div className="relative rounded-[28px] bg-white/90 backdrop-blur-sm shadow-2xl ring-1 ring-white/40">
          ...
          <div className="p-6 md:p-10">
            ...
            <ul className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {items.map((p) => (
                <li key={p.Id}>
                  <Link href={`/progress/${p.Id}`}
                        className="block relative rounded-3xl border border-gray-200 bg-white shadow-sm hover:shadow-md transition-all p-6 md:p-7">
                    <span aria-hidden className={`absolute top-4 right-4 h-3 w-3 rounded-full ${activeColor(p.Is_Active__c)}`} />
                    <div className="text-xl md:text-2xl font-semibold leading-snug text-gray-900">{p.Name}</div>
                    <dl className="mt-4 text-[15px] text-gray-700 space-y-1.5">
                      <div className="flex gap-2"><dt className="w-32 text-gray-500">status:</dt><dd className="flex-1">{p.StageName || "—"}</dd></div>
                      <div className="flex gap-2"><dt className="w-32 text-gray-500">campus:</dt><dd className="flex-1">{p.Campus__r?.Name ?? p.Campus__c ?? "—"}</dd></div>
                      <div className="flex gap-2"><dt className="w-32 text-gray-500">study program:</dt><dd className="flex-1">{p.Study_Program__r?.Name ?? p.Study_Program__c ?? "—"}</dd></div>
                      <div className="flex gap-2"><dt className="w-32 text-gray-500">test schedule:</dt><dd className="flex-1">{formatSFDateTime(p.Test_Schedule__c)}</dd></div>
                    </dl>
                  </Link>
                </li>
              ))}
