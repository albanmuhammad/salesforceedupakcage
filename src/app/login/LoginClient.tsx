...
export default function LoginClient() {
  ...
  // tentukan role dari query (?role=school|university)
  const role = (qs.get("role") || "university") as Role;
  ...
  // ===== UI presets per role (termasuk logoSrc) =====
  const ui = useMemo(() => {
    if (role === "school") {
      return {
        logoClass: "absolute top-2 left-2 md:top-3 md:left-3 h-28 w-auto drop-shadow",
        bgClass: "bg-gradient-to-br from-emerald-400 via-green-500 to-emerald-600",
        heroSrc: "/student4.png",
        logoSrc: "/Logo-MetroMini-transparent.png", // logo Metro Mini
    ...
    return {
      ...
      logoSrc: "/Logo-MetroSeven-removebg-preview.png", // logo Metro Seven
      brandName: "Metro Seven",
    };
  }, [role]);

  // URL tujuan ketika logo diklik
  const registrationUrl =
    process.env.NEXT_PUBLIC_REGISTRATION_WEB_URL || "/";

  return (
    <div className={`min-h-screen relative flex items-center justify-center ${ui.bgClass}`}>
      {/* Logo kiri atas -> bisa diklik untuk kembali ke halaman registrasi */}
      <a
        href={registrationUrl}
        aria-label={`Back to ${ui.brandName} main site`}
        title={`Back to ${ui.brandName} main site`}
        className="focus:outline-none focus:ring-2 focus:ring-white/70 rounded-md"
      >
        <img
          src={ui.logoSrc}
          alt={ui.brandName}
          className={ui.logoClass}
          draggable={false}
        />
      </a>
